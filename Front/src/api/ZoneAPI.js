...
class ZoneAPI {
  static async fetchData(endpoint, method = "GET", data = null) {
    try {
      ...
      static async getZoneAll() {
        try {
          const token = GlobalVar.getToken();
          const endpoint = "/api/zone/get-all";
      ...
      static async getZoneByID(zn_id) {
        try {
          const token = GlobalVar.getToken();
          const endpoint = `/api/zone/get-by-id/${zn_id}`;
      ...
      static async createZone(payload) {
        try {
          const token = GlobalVar.getToken();
          const endpoint = "/api/zone/create";
      ...
      static async updateZone(zn_id,formData) {
        try {
          const token = GlobalVar.getToken();
          const endpoint = `/api/zone/update/${zn_id}`;
      ...
      static async deleteZone(zn_id) {
        try {
          const token = GlobalVar.getToken();
          const endpoint = `/api/zone/delete/${zn_id}`;
    ...
    static validateHeaders(expectedHeaders, actualHeaders) {
      ...
      return matchingHeaders.length >= 4; // ถ้ามี headers ที่ตรงกันตั้งแต่ 7 ขึ้นไป ถือว่าผ่านการตรวจสอบ
    }
    ...
    static async importFile(file) {
      try {
          ...
          const expectedHeaders = ["รหัสZone", "ชื่อZone", "โรงงาน", "คลัง","หมายเหตุ"];
          ...
          const formattedData = jsonData.slice(1)
              .map(col => ({
                  "รหัสZone": col[0]?.toString().trim() || "",
                  "ชื่อZone": col[1]?.toString().trim() || "",
                  "โรงงาน": col[2]?.toString().trim() || "",
                  "คลัง": col[3]?.toString().trim() || "",
                  "หมายเหตุ": col[4]?.toString().trim() || "",
              }))
              .filter(col => col["รหัสZone"] !== ""); // ✅ กรองแถวที่ไม่มีรหัสออก
          ...
          // ✅ ส่ง JSON ไปยัง API
          const endpoint = "/api/zone/create-json";
