...
class UnitAPI {
  static async fetchData(endpoint, method = "GET", data = null) {
    try {
      ...
      static async getUnitAll() {
        try {
          const token = GlobalVar.getToken();
          const endpoint = "/api/unit/get-all";
          ...
          console.error("Error in unit:", error);
          throw error;
        }
      }

      static async getUnitByID(unit_id) {
        try {
          const token = GlobalVar.getToken();
          const endpoint = `/api/unit/get-by-id/${unit_id}`;
      ...
      static async createUnit(payload) {
        try {
          const token = GlobalVar.getToken();
          const endpoint = "/api/unit/create";
      ...
      static async updateUnit(unit_id,formData) {
        try {
          const token = GlobalVar.getToken();
          const endpoint = `/api/unit/update/${unit_id}`;
      ...
      static async deleteUnit(unit_id) {
        try {
          const token = GlobalVar.getToken();
          const endpoint = `/api/unit/delete/${unit_id}`;
    ...
    static validateHeaders(expectedHeaders, actualHeaders) {
      ...
      static async importFile(file) {
        try {
            ...
            const expectedHeaders = ["หน่วย", "หน่วย(คำย่อ)", "Unit", "Unit(Abbreviation)"];
            ...
            const formattedData = jsonData.slice(1)
                .map(col => ({
                    "หน่วย": col[0]?.toString().trim() || "",
                    "หน่วย(คำย่อ)": col[1]?.toString().trim() || "",
                    "Unit": col[2]?.toString().trim() || "",
                    "Unit(Abbreviation)": col[3]?.toString().trim() || "",
                }))
                .filter(col => col["หน่วย"] !== ""); // ✅ กรองแถวที่ไม่มีรหัสออก
            ...
            // ✅ ส่ง JSON ไปยัง API
            const endpoint = "/api/unit/create-json";
