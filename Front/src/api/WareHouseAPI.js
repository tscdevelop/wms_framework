  ...
  static async fetchData(endpoint, method = "GET", data = null) {
    try {
      ...
      static async getWareHouseAll() {
        try {
          const token = GlobalVar.getToken();
          const endpoint ="/api/warehouse/get-all";
          ...
          console.error("Error in warehouse:", error);
          throw error;
        }
      }

      static async getWareHouseByID(wh_id) {
        try {
          const token = GlobalVar.getToken();
          const endpoint = `/api/warehouse/get-by-id/${wh_id}`;
      ...
      static async createWareHouse(payload) {
        try {
          const token = GlobalVar.getToken();
          const endpoint = "/api/warehouse/create";
      ...
      static async updateWareHouse(wh_id,formData) {
        try {
          const token = GlobalVar.getToken();
          const endpoint = `/api/warehouse/update/${wh_id}`;
      ...
      static async deleteWareHouse(wh_id) {
        try {
          const token = GlobalVar.getToken();
          const endpoint = `/api/warehouse/delete/${wh_id}`;
      ...
      static validateHeaders(expectedHeaders, actualHeaders) {
        ...
        static async importFile(file) {
          try {
              ...
              const expectedHeaders = ["รหัสคลัง", "ชื่อคลัง", "ประเภทคลัง", "โรงงาน"];
              ...
              const formattedData = jsonData.slice(1)
                  .map(col => ({
                      "รหัสคลัง": col[0]?.toString().trim() || "",
                      "ชื่อคลัง": col[1]?.toString().trim() || "",
                      "ประเภทคลัง": col[2]?.toString().trim() || "",
                      "โรงงาน": col[3]?.toString().trim() || "",
                  }))
                  .filter(col => col["รหัสคลัง"] !== ""); // ✅ กรองแถวที่ไม่มีรหัสออก
              ...
              // ✅ ส่ง JSON ไปยัง API
              const endpoint = "/api/warehouse/create-json";
