...
const NotiShelf = () => {
  const [Noti, setNoti] = useState([]); // ข้อมูลทั้งหมด
  ...
  const fetchDataAll = async () => {
    try {
      const response = await NotificationAPI.getNotifShelfAll();
      // console.log("API Response:", response);
  
      if (response.isCompleted && Array.isArray(response.data.details)) {
        setNoti(response.data.details);  // ✅ ใช้ `data.details`
    ...
    <DashboardLayout>
      ...
      <MDBox p={2}>
        <MDBox mt={2} ml={5}>
          ...
          แจ้งเตือน / แจ้งเตือนอายุการเก็บรักษา (Shelf Life)
          </MDTypography>
        </MDBox>
        <MDBox mt={5}>
          <Card>
            ...
            <TableComponent
                columns={columns}
                data={Array.isArray(Noti) ? Noti : []}  // ✅ ป้องกัน error
                idField="notif_id"
                showActions={false}
                userRole={role}
                searchableColumns={[
