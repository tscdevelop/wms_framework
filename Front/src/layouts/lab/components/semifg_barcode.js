...
import InBoundSemiFGAPI from "api/InBoundSemiFGAPI"; // Import API
import html2canvas from "html2canvas";
import jsPDF from "jspdf";
import { QRCodeCanvas } from "qrcode.react"; // Import QRCodeCanvas
import ButtonComponent from "./ButtonComponent";
const PrintBarCodeSemiModal = ({ open, onClose, inbsemi_id }) => {
  ...
  const generatePDF = async () => {
    ...
    setTimeout(async () => {
      try {
        ...
        // const pdfHeight = (canvas.height * pdfWidth) / canvas.width; // คำนวณอัตราส่วน
        ...
        const imageWidth = contentWidth * 0.6; // ลดขนาดรูปภาพให้เล็กลง (80% ของความกว้าง)
        const imageHeight = contentHeight * 0.6; // ลดขนาดความสูงให้เป็น 80%
  
        pdf.addImage(imgData, "PNG", margin, margin, imageWidth, imageHeight); // ใส่ภาพพร้อมระยะขอบ
  ...
  const fetchLabelData = async () => {
    try {
      const response = await InBoundSemiFGAPI.getInBoundByID(inbsemi_id);
  ...
  const qrData = labelData
    ? JSON.stringify({
        code: labelData.inbsemi_code || "-",
        name: labelData.semiifm_name || "-",
        lot: labelData.inbsemi_lot || "-",
            ...
            <Box sx={{ padding: 2 }}>
              <Table
                sx={{
                  ...
                  "& .MuiTableCell-root": {
                    border: "2px solid gray", // เส้นขอบสีดำ
              ...
              >
                <TableBody>
                  <TableRow>
                    <TableCell
                      align="left"
                      sx={{ backgroundColor: "#F8D7A7", fontWeight: "bold", width: "200px" }}
                    >
                      รหัส
                    </TableCell>
                    <TableCell align="left" sx={{ width: "300px" }}>
                      {labelData.inbsemi_code || "-"}
                    </TableCell>
                  </TableRow>
                  <TableRow>
                    <TableCell
...
export default PrintBarCodeSemiModal;
