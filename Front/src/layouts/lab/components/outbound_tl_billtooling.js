...
const PrintTLBillComponent = ({ open, onClose, data, outbtl_id }) => {
  ...
  useEffect(() => {
    if (!data && outbtl_id) {
      const fetchData = async () => {
        try {
          const response = await OutBoundToolingAPI.getOutBoundTLReqByID(outbtl_id);
          if (response?.data) {
  ...
  const generatePDF = async () => {
    ...
    try {
      ...
      if (displayData.outbtl_id) {
        const payload = { withdr_date: today_date_time };
        await OutBoundToolingAPI.updateOutbDateTL(displayData.outbtl_id, payload);
            ...
            <tbody>
              <tr>
                ...
                <td style={{ textAlign: "center", border: "none" }}>
                  ...
                  <div style={{ marginBottom: "4px" }}>
                    <strong>ชื่อผู้เบิก(NAME):</strong> {outbtl_issued_by}
                  </div>
...
export default PrintTLBillComponent;