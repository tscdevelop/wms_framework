...
const ConfirmSending = () => {
 ...
 const fetchDataAll = async () => {
  try {
    let response;
    if (tabIndex === 1) {
      // สำหรับ "รายการส่งของทั้งหมด" ส่ง withdrawStatus:true ด้วย
      response = await OutBoundFGAPI.getOutBoundFGAll("APPROVED",true);
    } else {
      // สำหรับ "รายการเบิก Semi FG ทั้งหมด" ไม่ต้องส่ง withdrawStatus
      response = await OutBoundFGAPI.getOutBoundFGAll("APPROVED");
    }
                    ...
                    <TableComponent
                      columns={[
                        ...
                        { field: "withdr_time", label: "เวลา", width: "5%" },
                        {
                          ...
                          width: "10%",
                          render: (row) => {
                            const outbfg_code = row.outbfg_code;
                            return (
                              <a
                                href={`/confirm-wd?outbfg_id=${encodeURIComponent(row.outbfg_id)}`}
                              >
                                <MDTypography variant="subtitle2" sx={{ color: "blue" }} > {outbfg_code}</MDTypography>
                              </a>
                            );
                          },
                        },
                        {
                          ...
                          render: (row) => {
                            ...
                            if (row.outbfgitm_withdr_status === "COMPLETED") {
                              statusText = "เบิกสำเร็จ";
                              statusColor = "#4CAF50"; // สีเขียว
                            } else if (row.outbfgitm_withdr_status === "PARTIAL") {
                              statusText = "เบิกไม่ครบ";
                              statusColor = "#FFA500"; // สีส้ม (ปรับได้ตามต้องการ)
                            } else if (row.outbfgitm_withdr_status === "PENDING") {
...

                        {
                          ...
                          render: (row) => (
                            <a
                              href={`/confirm-wd?outbfg_id=${encodeURIComponent(
                                row.outbfg_id
                              )}&status=view`}
                      ...
                      columns={[
                        ...
                        },
                        {
                          ...
                          render: (row) => {
                            ...
                            return (
                              <a
                                href={`/confirm-deli?outbfg_id=${encodeURIComponent(
                                  row.outbfg_id
                                )}`}
                              >
                                <MDTypography variant="subtitle2" sx={{ color: "blue" }} > {outbfg_code}</MDTypography>
                        ...
                        { field: "outbfg_details", label: "รายละเอียด", width: "20%" },
                        {
...

                        {
                          ...
                          render: (row) => (
                            <a
                              href={`/confirm-deli?outbfg_id=${encodeURIComponent(row.outbfg_id)}&status=view`}
                              style={{ textDecoration: "none", color: "blue", fontWeight: "bold" }}
                            >
                              <MDTypography variant="subtitle2" sx={{ color: "blue" }} >ดูรายละเอียด</MDTypography>
                            </a>
