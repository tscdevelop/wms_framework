...
const fetchDataAll = async () => {
    try{
      const response = await SemiAPI.getSemiAll();
  ...
  const handleEdit = async (semi_id) => {
    try {
      const response = await SemiAPI.getSemiByID(semi_id);
      if (response.isCompleted) {
        const Data = response.data;
        // อัปเดต ZoneForm
        setForm(Data);
  ...
  const validateForm = () => {
    ...
    if (!Form.semi_code || !Form.semi_code.trim()) {
      newErrors.semi_code = "กรุณากรอกรหัส Semi";
    }
  
    if (!Form.semi_type || !Form.semi_type.trim()) {
      newErrors.semi_type = "กรุณากรอกประเภท Semi";
    }
...
const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (mode === "add") {
        ...
        const payload = {
            semi_code: Form.semi_code || "",
            semi_type: Form.semi_type || "",
            semi_remark: Form.semi_remark || "",
        };
        response = await SemiAPI.createSemi(payload);
      } else {
        // FormData สำหรับอัปเดตข้อมูล
        const formData = new FormData();
        formData.append("semi_code", Form.semi_code || "");
        formData.append("semi_type", Form.semi_type || "");
        formData.append("semi_remark", Form.semi_remark || "");
        response = await SemiAPI.updateSemi(Form.semi_id, formData); // ใช้ API updateSupplier
      }
  ...
  const handleDelete = async () => {
    try {
      const response = await SemiAPI.deleteSemi(deleteCode);
      if (response.isCompleted) {
        setAlert({
          show: true,
          type: "success",
   ...
   const handleSubmitImport = async () => {
     ...
     try {
       const response = await SemiAPI.importFile(selectedFile);
       if (response.isCompleted) {
         setAlert({
           show: true,
           type: "success",
          ...
          <MDBox mt={5}>
            <Card>
              ...
              <Card>
                <TableComponent
                  columns={[
                    { field: "semi_code", label: lang.msg("semi.semi_code"), width: "5%" },
                    { field: "semi_type", label: lang.msg("semi.semi_type"), width: "10%" },
                    { field: "semi_remark", label: lang.msg("semi.semi_remark"), width: "5%" },
                  ]}
                  data={semiAll}
                  idField="semi_id"
                  onEdit={(id) => {
