...
const InventoryRaw = () => {
  ...
  const fetchDataAll = async () => {
    try {
      const response = await InventoryAPI.getInventoryRaw();
  ...
  const columns = [
    { field: "inbrm_code", label: "รหัสสินค้า", width: "13%" },
    { field: "rmifm_name", label: "ชื่อสินค้า", width: "13%" },
    { field: "inbrm_quantity", label: "จำนวนคงเหลือในคลัง", width: "13%" },
    { field: "inbrm_lot", label: "Lot.", width: "13%" },
  ...
  const handleExport = async () => {
    try {
      ...
      const response = await InventoryAPI.ExportInvRaw(
