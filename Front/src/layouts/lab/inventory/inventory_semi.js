...
const InventorySemi = () => {
  ...
  const fetchDataAll = async () => {
    try {
      const response = await InventoryAPI.getInventorySemi();
  ...
  const columns = [
    { field: "inbsemi_code", label: "รหัสสินค้า", width: "13%" },
    { field: "semiifm_name", label: "ชื่อสินค้า", width: "13%" },
    { field: "inbsemi_quantity", label: "จำนวนคงเหลือในคลัง", width: "13%" },
    { field: "inbsemi_lot", label: "Lot.", width: "13%" },
  ...
  const handleExport = async () => {
    try {
      ...
      const response = await InventoryAPI.ExportInvSemi(
