...
const InventoryFG = () => {
  ...
  const fetchDataAll = async () => {
    try {
      const response = await InventoryAPI.getInventoryFg();
  ...
  const columns = [
    { field: "inbfg_code", label: "รหัสสินค้า", width: "13%" },
    { field: "fgifm_name", label: "ชื่อสินค้า", width: "13%" },
    { field: "inbfg_quantity", label: "จำนวนคงเหลือในคลัง", width: "13%" },
    { field: "inbfg_lot", label: "Lot.", width: "13%" },
  ...
  const handleExport = async () => {
    try {
      ...
      const response = await InventoryAPI.ExportInvFg(
